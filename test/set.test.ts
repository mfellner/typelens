...
describe('set', () => {
  ...
  test('should work with objects', () => {
    expect(set(['x', 'y', 'z'])(42)({ x: { y: { z: 0 } } }).get()).toEqual({ x: { y: { z: 42 } } });
  });

  test('should work with arrays', () => {
    expect(set(1)(42)([1, 2, 3]).get()).toEqual([1, 42, 3]);
  });

  test('should work with mixed objects', () => {
    expect(set(['x', 1, 'z'])(42)({ x: [null, { z: 0 }] }).get()).toEqual({ x: [null, { z: 42 }] });
  });

  test('should create a non-existing path', () => {
    expect(set(['x', 0, 'z'])(42)({ a: 'a' }).get()).toEqual({ a: 'a', x: [{ z: 42 }] });
  });
